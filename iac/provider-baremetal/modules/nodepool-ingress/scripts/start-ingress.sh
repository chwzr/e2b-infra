#!/bin/bash
# Bootstrap script for the dedicated ingress VM (runs Traefik via Nomad).
# Dual-NIC: public (default gateway) + private (cluster traffic). Nomad/Consul
# advertise on the private IP so intra-cluster service discovery works.

set -e

exec > >(tee /var/log/start-ingress.log | logger -t start-ingress -s 2>/dev/console) 2>&1
...
# ---
# Consul Client (advertising on private IP so services register on private subnet)
# ---
...
echo "Ingress node setup complete."
