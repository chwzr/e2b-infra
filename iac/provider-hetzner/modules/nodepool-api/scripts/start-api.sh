#!/bin/bash
# Cloud-init script for API/ingress nodes on Hetzner Cloud.
# Configures and starts Consul client + Nomad client with API node pool.
...
# ---
# Get private IP (Hetzner private network interface)
# ---
PRIVATE_IP=$(ip -4 addr show ens10 | grep -oP '(?<=inet\s)\d+(\.\d+){3}' || hostname -I | awk '{print $1}')

# ---
# Docker configuration with custom registry
# ---
mkdir -p /root/docker
...
# ---
# Consul DNS resolution via systemd-resolved
# ---
...
cat > /opt/consul/config/default.json <<EOF
{
  ...
  "acl": {
    ...
    "tokens": {
      "default": "${CONSUL_DNS_REQUEST_TOKEN}"
    }
  },
  "telemetry": {
